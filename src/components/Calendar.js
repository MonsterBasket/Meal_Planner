...
function Calendar(){
    ...
    useEffect(_ => {
        ...
        let newDate;
        newDate = dates ? new Date(dates.thisYear, newMonth, 1) : today;
    ...
    function printTasks(id){
        return tasks.length === 0 ? null : tasks.filter(a => a.date === id).map(a => <div key={a.id} className="tasks">{a.name}</div>)
    ...
    return <div><h2>My Calendar</h2>
        {dates === null ? <h3>Loading Calendar...</h3>
            : <>
                ...
                <div className={`newTask${newTaskDate ? "visible" : "hidden"}`}>
                    <NewTask date={newTaskDate} userId={user.id}/>
                </div>
            </>}
    </div>
}
